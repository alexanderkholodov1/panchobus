...
/**
 * DemoSessionProvider — sesión de prototipo.
 *
 * Persiste el usuario activo en localStorage. Provee tres "logins demo":
 * estudiante, admin y chofer. Cuando Supabase Auth esté configurado, este
 * provider lee también la sesión real de Supabase y la prioriza.
 */
...
export function DemoSessionProvider({ children }: { children: React.ReactNode }) {
  ...
  const loginAs = useCallback(
    async (role: "estudiante" | "admin" | "chofer") => {
  ...
  const loginByCredentials = useCallback(
    async (email: string, _password: string): Promise<Usuario | null> => {
      const usuarios = await db.getUsuarios();
      const u = usuarios.find(
        (x) => x.correo_electronico.toLowerCase() === email.toLowerCase()
      );
      if (!u) return null;
  ...
  const register = useCallback(
    async (
      ...
      const nuevo: Usuario = {
        ...
        rol: "estudiante",
        estado: "activo", // demo: auto-activos
  ...
  return (
    <DemoSessionContext.Provider
      value={{ user, isLoading, loginAs, loginByCredentials, register, logout, refresh }}
    >
      {children}
    </DemoSessionContext.Provider>
  );
