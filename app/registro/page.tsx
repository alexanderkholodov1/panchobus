  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const u = await register({
        ...
        codigo_banner: form.codigo_banner, telefono: form.telefono,
        direccion: form.direccion, id_ruta: form.id_ruta ? Number(form.id_ruta) : null
      });
      toast({ title: "Cuenta creada", description: `Bienvenido, ${u.nombre}`, variant: "success" });
      ...
      <main className="flex-1 flex items-center justify-center px-4 py-8">
        <div className="w-full max-w-xl space-y-5">
          <div className="text-center space-y-1">
            ...
            <p className="text-muted text-sm">Tu libertad, comienza aquí.</p>
          </div>
          <Card>
            <CardBody>
              <form onSubmit={onSubmit} className="space-y-3.5">
                ...
                <div className="grid sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <Input id="email" type="email" placeholder="tu@estud.usfq.edu.ec" value={form.email} onChange={set("email")} required />
                  </div>
                  <div>
                    <Label htmlFor="banner">Código Banner</Label>
                    <Input id="banner" placeholder="00332509" value={form.codigo_banner} onChange={set("codigo_banner")} required maxLength={8} />
                  </div>
                </div>
                <div className="grid sm:grid-cols-2 gap-3">
                  <div>
                    <Label htmlFor="telefono">Teléfono</Label>
                    <Input id="telefono" placeholder="+593 99 123 4567" value={form.telefono} onChange={set("telefono")} required />
                  </div>
                  <div>
                    <Label htmlFor="direccion">Sector / barrio</Label>
                    <Input id="direccion" placeholder="Cumbayá" value={form.direccion} onChange={set("direccion")} />
                  </div>
                </div>
                <div>
                ...
                <div className="grid sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <Input id="pwd" type="password" value={form.password} onChange={set("password")} required minLength={6} />
                  </div>
                  <div>
                    <Label htmlFor="cpwd">Confirmar contraseña</Label>
                    <Input id="cpwd" type="password" value={form.confirmPassword} onChange={set("confirmPassword")} required minLength={6} />
                  </div>
                </div>
                {error && <p className="text-sm text-state-error bg-state-error/10 px-3 py-2 rounded-lg">{error}</p>}
                <Button type="submit" size="lg" className="w-full" loading={loading}>Crear cuenta</Button>
