...
export default function RutasPage() {
  ...
  useEffect(() => { db.getRutas().then(setRutas); }, []);

  const filtered = useMemo(() => rutas.filter((r) => {
    const matchQ = !q || r.nombre.toLowerCase().includes(q.toLowerCase()) ||
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input className="w-full h-10 pl-10 pr-3 rounded-lg border border-border bg-surface text-sm focus:outline-none focus:ring-2 focus:ring-primary/40"
              placeholder="Buscar por nombre, código o barrio…" value={q} onChange={(e) => setQ(e.target.value)} />
          </div>
          <div className="flex gap-2">
            {(["todas", "activa", "suspendida"] as const).map((f) => (
              <button key={f} onClick={() => setFiltro(f)}
                className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                  filtro === f ? "bg-primary text-primary-foreground" : "bg-surface border border-border hover:bg-surface-2"
                }`}>
        ...
        <p className="text-sm text-muted">{filtered.length} {filtered.length === 1 ? "ruta" : "rutas"} encontradas</p>
