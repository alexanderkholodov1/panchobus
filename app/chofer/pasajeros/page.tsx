      ...
      <div className="max-w-xl mx-auto px-4 sm:px-6 py-8 space-y-6">
          ...
          <Card>
            <CardBody className="space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium">{usadas} / {asignacion.cupos_disponibles}</span>
              </div>
              <div className="h-3 bg-surface-2 rounded-full overflow-hidden">
                <div className="h-full rounded-full bg-state-ok transition-all"
                  style={{ width: `${Math.min(100, (usadas / asignacion.cupos_disponibles) * 100)}%` }} />
              </div>
            </CardBody>
          </Card>
        )}
        ...
        <div className="space-y-2">
          ...
          {reservas.map((r) => {
            ...
            const Icon = r.estado === "usada" ? CheckCircle2 :
                         r.estado === "en_espera" ? Hourglass :
                         r.estado === "cancelada" ? XCircle : Clock;
            const color = r.estado === "usada" ? "text-state-ok" :
                          r.estado === "en_espera" ? "text-state-warn" :
                          r.estado === "cancelada" ? "text-state-error" : "text-primary";
            return (
                ...
                <CardBody>
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-sm">{u?.nombre ?? "Desconocido"}</p>
